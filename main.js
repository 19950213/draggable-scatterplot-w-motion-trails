//data url
var url = "https://raw.githubusercontent.com/RandomEtc/mind-gapper-js/master/data/",
    gdpFile = "gdp_per_capita_ppp.csv",
    populationFile = "indicator_gapminder_population.csv",
    lifeExpectancyFile = "life_expectancy_at_birth.csv",
...
d3.csv(url + gdpFile).then(function (gdpData) {
    d3.csv(url + populationFile).then(function (popData) {
        d3.csv(url + lifeExpectancyFile).then(function (lifeData) {
            d3.csv(url + regionFile).then(function (groupData) {
                gdp = gdpData;
                pop = popData;
                life = lifeData;
                group = groupData;
...
function start() {
    ...
    scaleX.domain([getMin("gdp"), getMax("gdp")]);
    scaleY.domain([getMin("life"), getMax("life")]);
